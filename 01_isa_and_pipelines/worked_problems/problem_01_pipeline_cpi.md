...
This example shows how the contributions stack up. Each stall source looks small individually, but they aggregate to a ~15% longer run time (a ~13% throughput loss, since 1/1.154 ≈ 0.87) on otherwise-ideal code.
...
**Branch prediction sensitivity:** If the predictor accuracy drops from 92% to 85%, the branch contribution becomes 0.18 × 0.15 × 2 = 0.054, a 0.025 increase — comparable to the load-use cost. A predictor improvement from 92% to 97% saves 0.18 × 0.05 × 2 = 0.018 CPI.
